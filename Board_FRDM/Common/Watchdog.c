...
#define WDT_TASK_MS_HIGH_CNT    900 /* task counter must be below this value */
...
static portTASK_FUNCTION(WatchdogTask, pvParameters) {
  ...
  LED_Green_Off(); /* Turn off watchdog LED */
  for(;;) {
    state = 0x5555;
    wdt_a();
...
void WDT_Init(void) {
  ResetTaskCntr();
  LED_Green_On(); /* turn red LED on. In case of watchdog reset, we should see the RED LED */
  if (FRTOS1_xTaskCreate(WatchdogTask, (signed portCHAR *)"WDog", configMINIMAL_STACK_SIZE, NULL, tskIDLE_PRIORITY, NULL) != pdPASS) {
    for(;;){} /* error */
  }
}
